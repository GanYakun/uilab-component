  ...
  switch (payloadInfo.detectedPayloadKind) {
    case PAYLOADTYPE_VALUE:
      if (payloadInfo.type !== null) {
        return addMetadataToJsonMinimalPayloadEntity(
          data,
          payloadInfo,
          baseURI,
          model,
    ...
    case PAYLOADTYPE_COLLECTION:
      return addMetadataToJsonMinimalPayloadCollection(
        data,
        model,
        payloadInfo,
        baseURI,
...
function addMetadataToJsonMinimalPayloadFeed(data, model, feedInfo, baseURI, recognizeDates) {
  ...
  for (i = 0, len = items.length; i < len; i++) {
    var item = items[i];
    if (defined(item['@odata.type'])) {
      // in case of mixed feeds
      ...
      entry = addMetadataToJsonMinimalPayloadEntity(
        item,
        entryInfo,
        baseURI,
        model,
 ...
 * @param {Boolean} recognizeDates - Flag indicating whether datetime literal strings should be converted to JavaScript Date objects.
 */
function addMetadataToJsonMinimalPayloadCollection(
  data,
  model,
  collectionInfo,
  baseURI,
...
function jsonIsPrimitiveType(typeName) {
  return (
    isPrimitiveEdmType(typeName) || isGeographyEdmType(typeName) || isGeometryEdmType(typeName)
  );
}

var jsonHandler = oDataHandler.handler(
  jsonParser,
  jsonSerializer,
  jsonMediaType,
