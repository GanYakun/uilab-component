                ...
                for (let b of record) {
                    ...
                    switch (type) {
                        ...
                        case 'UI.DataFieldForAction':
                            ...
                            //判断是行内还是头部
                            if (Inline === 'true') {
                                result.inLineBtns.push({ Action: ActionData, Label, type, Url, IconUrl })
                            } else {
                                result.headerBtns.push({ Action: ActionData, Label, type, Url, IconUrl })
                            }
                            break
        ...
        currentColumns.map((item: { path: any; type: any; value: any; show: any; url: any; Criticality: any, Url: any }) => {
            ...
            switch (type) {
                ...
                case 'UI.DataFieldWithUrl':
                    ...
                    break;
                case 'UI.DataPoint':
                    const { Criticality: DataPointCriticality }=value
                    DataPointCriticality&&fieldArr.push(DataPointCriticality)
                    fieldArr.push(path)
                    break;
...
export const getConfig = async (params: { entitySet: any; qualifier: any }) => {
    ...
    const restrictions = Utils.getObjectRestrictions(currentAnnotations)
